...
function createExerciseCard(name) {
  ...
  div.innerHTML = `
    <div class="exercise-card-header">
      <input type="text" class="input-field exercise-name-input hw-target" placeholder="種目名（タップで手書き）" id="name-${id}" value="${esc(name)}" data-hw-hint="テキスト">
...
function addSet(exId, reps = '', weight = '', memo = '') {
  ...
  row.innerHTML = `
    ...
    <input type="text" inputmode="decimal" class="set-input" placeholder="回" id="reps-${sid}" value="${esc(reps)}">
    <input type="text" inputmode="decimal" class="set-input" placeholder="kg" id="weight-${sid}" value="${esc(weight)}">
    <input type="text" class="set-input" placeholder="メモ" id="memo-${sid}" value="${esc(memo)}">
  `;
